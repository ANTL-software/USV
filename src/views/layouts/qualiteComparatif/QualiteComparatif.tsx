...
import { BackToTop, ComparisonSection, Header, SubNav } from '../../components/index.ts';
...
function QualiteComparatif() {
  ...
  return <div id="qualiteComparatif"><Header /><SubNav /><main className="comparison__wrapper">
    <button className="comparison__back" onClick={vm.navigateBack}><IoArrowBack /> Qualité</button>
    ...
    <form className="comparison__filters" onSubmit={(event) => { event.preventDefault(); vm.apply(); }} aria-label="Filtres du comparatif">
      <label>Campagne<select id="comparison-campaign" value={vm.draft.campaign} onChange={(event) => vm.changeCampaign(event.target.value)} disabled={!vm.options}><option value="0" disabled>Choisir une campagne</option>{vm.options?.campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select></label>
      <label>Commercial<select value={vm.draft.agent ?? ''} onChange={(event) => vm.changeAgent(event.target.value)}><option value="">Tous les commerciaux</option>{vm.agents.map((agent) => <option value={agent.id} key={agent.id}>{agent.name}</option>)}</select></label>
      <label>Mois analysé<input type="month" value={vm.draft.month} max={vm.maxMonth} min="2000-01" required onChange={(event) => vm.changeMonth(event.target.value)} /></label>
      ...
      <label>Mois de référence<input type="month" value={vm.draft.reference} max={vm.maxMonth} min="2000-01" required onChange={(event) => vm.changeReference(event.target.value)} /></label>
      <label>Base de comparaison<select value={vm.draft.mode} onChange={(event) => vm.changeMode(event.target.value)}><option value="aligned">Même nombre de jours terminés</option><option value="full">Mois entiers / en cours à date</option></select></label>
      ...
      <p className="comparison__filter-help">À durée égale, seuls les jours calendaires terminés communs aux deux mois sont comparés. Cela ne neutralise pas les différences de jours ouvrés ou de saisonnalité.</p>
    ...
    {vm.data && <>
      ...
      <div className="comparison__kpis">{vm.cards.map((card) => <article className="comparison__kpi" key={card.key} title={card.description}><span>{card.label}</span><strong>{card.value}</strong><div><b>{card.delta}</b><small>Réf. {card.previous}</small></div></article>)}</div>
      <aside className="comparison__signals"><h2>Points à examiner</h2>{vm.signals.map((signal) => <p key={signal}>{signal}</p>)}</aside>
