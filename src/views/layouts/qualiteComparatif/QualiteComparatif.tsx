...
function QualiteComparatif() {
  ...
  return <div id="qualiteComparatif"><Header /><SubNav /><main className="comparison__wrapper">
    ...
    <form className="comparison__filters" onSubmit={(event) => { event.preventDefault(); vm.apply(); }} aria-label="Filtres du comparatif">
      ...
      <label>Base de comparaison<select aria-label="Base de comparaison" value={vm.draft.mode} onChange={(event) => vm.changeMode(event.target.value)}><option value="business">Même nombre de jours ouvrés (recommandé)</option><option value="aligned">Même nombre de jours calendaires</option><option value="full">Mois entiers / en cours à date</option></select></label>
      ...
      <p className="comparison__filter-help"><strong>Le mode recommandé compare le même nombre de jours ouvrés, hors week-ends et jours fériés français.</strong> Le jour en cours est inclus ; son équivalent dans l’autre mois est arrêté à la même heure. <em>Les modes calendaires restent disponibles pour analyser une activité exceptionnelle le week-end.</em></p>
    ...
    {vm.data && <>
      ...
      {vm.emptyDays && <p className="comparison__notice">Aucun jour comparable pour l’instant. En mode jours ouvrés, cela peut arriver au début d’un mois pendant un week-end ou un jour férié.</p>}
