  ...
  return (
    <>
      <div className="vigieView__grid vigieView__grid--quality">
        <section className="vigieView__panel">
          ...
          <dl className="vigieView__quality-list">
            <div><dt>Fiches campagne</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.fiches_file)}</dd></div><div><dt>Sans segment exploitable</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.sans_segment_exploitable)}</dd></div><div><dt>Sans contact nommé</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.sans_contact_nomme)}</dd></div><div><dt>Sans code NAF</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.sans_code_naf)}</dd></div><div><dt>Sans code postal</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.sans_code_postal)}</dd></div><div><dt>Mobiles identifiés</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.mobiles)}</dd></div><div><dt>Doublons / blacklist</dt><dd>{formatVigieNumber(snapshot.qualite_fichier.doublons + snapshot.qualite_fichier.blacklists)}</dd></div>
          </dl>
      ...
      <div className="vigieView__grid vigieView__grid--signals">
        <section className="vigieView__panel"><div className="vigieView__panel-title"><IoTimeOutline /><div><h2>Créneaux de contact</h2><p>Historique complet de la campagne, indépendant du filtre de période. Contact humain confirmé par un ProgPA de 1 à 5.</p></div></div>{state.topContactHours.length === 0 ? <p className="vigieView__empty-copy">Aucun appel enregistré sur la campagne.</p> : <div className="vigieView__hour-grid">{state.topContactHours.map((hour) => <article key={hour.heure}><strong>{String(hour.heure).padStart(2, '0')}h–{String(hour.heure + 1).padStart(2, '0')}h</strong><span>{formatVigiePercent(hour.taux_contact_humain)}</span><small>{formatVigieNumber(hour.contacts_humains)} contacts · {formatVigieNumber(hour.appels)} appels</small></article>)}</div>}</section>
        <section className="vigieView__panel"><div className="vigieView__panel-title"><IoCalendarOutline /><div><h2>Journées les plus pertinentes</h2><p>Historique complet de la campagne, indépendant du filtre de période. Moyenne des taux quotidiens confirmés par ProgPA.</p></div></div>{state.topContactDays.length === 0 ? <p className="vigieView__empty-copy">Aucun appel enregistré sur la campagne.</p> : <div className="vigieView__hour-grid">{state.topContactDays.map((day) => <article key={day.jour}><strong>{getVigieWeekdayLabel(day.jour)}</strong><span>{formatVigiePercent(day.taux_contact_humain)}</span><small>Moyenne de {formatVigieWeekdayObservationCount(day.jour, day.jours_observes)} · {formatVigieNumber(day.appels)} appels</small></article>)}</div>}</section>
      </div>
