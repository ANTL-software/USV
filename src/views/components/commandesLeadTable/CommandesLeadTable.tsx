...
export function CommandesLeadTable({ viewModel }: CommandesLeadTableProps): ReactElement | null {
  ...
  return <>
    <div className="commandesList__table-wrapper"><table>
      <thead><tr><th>Réf.</th><th>Date d'émission</th><th>Date de qualification</th><th>Rendez-vous</th><th>Client</th><th>Interlocuteur</th><th>5+ salariés</th><th>Agent</th><th>Statut</th></tr></thead>
      <tbody>{commandes.leadClients.map((lead) => <tr key={lead.id_lead} onClick={() => viewModel.navigateToLead(lead.id_lead)} className={['commandesList__row--clickable', !lead.fiche_envoyee_at ? 'commandesList__row--email-pending' : ''].filter(Boolean).join(' ')}>
        <td>{formatLeadClientReference(lead.id_lead)}</td><td><span className={isDateBeforePeriod(lead.created_at, periodStart) ? 'commandesList__date commandesList__date--emission-previous' : undefined}>{formatCommandesDate(lead.created_at)}</span></td><td>{lead.date_qualification ? <span className={isDateAfterPeriod(lead.date_qualification, periodEnd) ? 'commandesList__date commandesList__date--validation-later' : undefined}>{formatCommandesDate(lead.date_qualification)}</span> : '—'}</td><td>{formatLeadSlot(lead.date_rdv, lead.heure_rdv)}</td><td>{getLeadProspectName(lead)}</td><td>{getLeadInterlocuteur(lead)}</td><td>{lead.entreprise_plus_de_cinq_salaries ? 'Oui' : 'Non'}</td><td>{getLeadAgentName(lead)}</td>
