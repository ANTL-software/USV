...
import { formatCommandesDate, formatMontant, getVenteAgentName, getVenteProspectName } from '../../../utils/scripts/index.ts';
...
export function CommandesSaleTable({ viewModel }: CommandesSaleTableProps): ReactElement | null {
  ...
  return <>
    <div className="commandesList__table-wrapper"><table>
      <thead><tr><th>Réf.</th><th>Date</th><th>Client</th><th>Agent</th><th>Montant</th>{!commandes.isCorbeille && <th>Statut</th>}{!commandes.isCorbeille && <th>Paiement</th>}<th className="commandesList__actions-heading">Actions</th></tr></thead>
      <tbody>{commandes.ventes.map((vente) => <tr key={vente.id_vente} onClick={() => { if (!vente.soft_deleted) viewModel.navigateToSale(vente.id_vente); }} className={[!commandes.isCorbeille ? 'commandesList__row--clickable' : '', vente.soft_deleted ? 'commandesList__row--deleted' : ''].filter(Boolean).join(' ')}>
        <td>{vente.reference_doc ?? `#${vente.id_vente}`}</td><td>{formatCommandesDate(vente.date_vente)}</td><td>{getVenteProspectName(vente)}</td><td>{getVenteAgentName(vente)}</td><td className="commandesList__montant">{formatMontant(vente.montant_total)}</td>
