...
import { formatCommandesDate, formatMontant, getVenteAgentName, getVenteProspectName, isDateAfterPeriod, isDateBeforePeriod, isFrigoReminderDue } from '../../../utils/scripts/index.ts';
...
export function CommandesSaleTable({ viewModel }: CommandesSaleTableProps): ReactElement | null {
  ...
  return <>
    <div className="commandesList__table-wrapper"><table>
      ...
      <tbody>{commandes.ventes.map((vente) => <tr key={vente.id_vente} onClick={() => { if (!vente.soft_deleted) viewModel.navigateToSale(vente.id_vente); }} className={[!commandes.isCorbeille ? 'commandesList__row--clickable' : '', vente.soft_deleted ? 'commandesList__row--deleted' : '', !vente.soft_deleted && !vente.email_envoye_at ? 'commandesList__row--email-pending' : ''].filter(Boolean).join(' ')}>
