...
import { formatCommandesDate, formatMontant, getSaleEmailDispatchRowClass, getVenteAgentName, getVenteProspectName, isDateAfterPeriod, isDateBeforePeriod, isFrigoReminderDue } from '../../../utils/scripts/index.ts';
...
export function CommandesSaleTable({ viewModel }: CommandesSaleTableProps): ReactElement | null {
  ...
  return <>
    <div className="commandesList__table-wrapper"><table>
      ...
      <tbody>{commandes.ventes.map((vente) => <tr key={vente.id_vente} onClick={() => { if (!vente.soft_deleted) viewModel.navigateToSale(vente.id_vente); }} className={[!commandes.isCorbeille ? 'commandesList__row--clickable' : '', vente.soft_deleted ? 'commandesList__row--deleted' : '', !vente.soft_deleted ? getSaleEmailDispatchRowClass({ statutVente: vente.statut_vente, prospectEmailSentAt: vente.email_envoye_at, signedOrderSentAt: vente.bon_commande_signe_envoye_at }) : ''].filter(Boolean).join(' ')}>
        <td><span className="commandesList__reference">{vente.reference_doc ?? `#${vente.id_vente}`}{isFrigoReminderDue(vente) && <span className="commandesList__frigo-alert-dot" title="Commande frigo à relancer" />}</span></td><td><span className={isDateBeforePeriod(vente.date_vente, periodStart) ? 'commandesList__date commandesList__date--emission-previous' : undefined}>{formatCommandesDate(vente.date_vente)}</span></td><td>{vente.date_acceptation ? <span className={isDateAfterPeriod(vente.date_acceptation, periodEnd) ? 'commandesList__date commandesList__date--validation-later' : undefined}>{formatCommandesDate(vente.date_acceptation)}</span> : '—'}</td><td>{getVenteProspectName(vente)}</td><td>{getVenteAgentName(vente)}</td><td className="commandesList__montant">{formatMontant(vente.montant_total)}</td>
