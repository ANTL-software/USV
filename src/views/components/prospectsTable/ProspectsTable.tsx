  ...
  return (
    <tr onClick={() => onSelect(prospect)} className={isProspectGloballyFlagged(prospect) ? 'prospectsView__row--alert' : ''}>
      ...
      <td>{campaignMode ? (
        ...
        <div className="prospectsView__relations">{prospect.relations_commerciales.map((relation) => <span key={relation.id_relation ?? `${relation.id_campagne}-${relation.statut_relation}`} className={`badge ${getProspectRelationBadgeClass(relation)}`}>{getProspectRelationLabel(relation)}{relation.campagne ? ` · ${relation.campagne.nom_campagne}` : ''}</span>)}</div>
