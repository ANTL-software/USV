...
interface IncidentQualificationFormProps { viewModel: IncidentQualificationViewModel }

export function IncidentQualificationForm({ viewModel }: IncidentQualificationFormProps): ReactElement {
  ...
  return (
    <form className="incidents__form" onSubmit={viewModel.submit}>
      <div className="incidents__grid">
        <div className="incidents__field"><label htmlFor="secteur">Secteur</label><IncidentSelect inputId="secteur" options={INCIDENT_SECTEUR_OPTIONS} value={form.secteur} onChange={(value) => updateField('secteur', (value ?? 'software') as IncidentSecteur)} disabled={isSaving} /></div>
        ...
        <div className="incidents__field"><label htmlFor="classification">Classification</label><input id="classification" list="classification-options" value={form.classification} onChange={(event) => updateField('classification', event.target.value)} disabled={isSaving} /><datalist id="classification-options">{INCIDENT_CLASSIFICATION_OPTIONS.map((option) => <option key={option} value={option} />)}</datalist></div>
        <div className="incidents__field"><label htmlFor="id_intervenant">Intervenant</label><IncidentSelect inputId="id_intervenant" options={viewModel.employeeOptions} value={form.id_intervenant || null} onChange={(value) => updateField('id_intervenant', value ?? '')} placeholder="Non assigné" disabled={isSaving} isClearable /></div>
        <div className="incidents__field incidents__field--wide"><label htmlFor="tags">Tags</label><input id="tags" value={form.tags} onChange={(event) => updateField('tags', event.target.value)} disabled={isSaving} /></div>
      </div>
      <div className="incidents__field incidents__field--wide"><label htmlFor="commentaire_qualification">Commentaire de qualification</label><textarea id="commentaire_qualification" value={form.commentaire_qualification} onChange={(event) => updateField('commentaire_qualification', event.target.value)} rows={4} disabled={isSaving} /></div>
      <div className="incidents__actions"><button type="submit" className="incidents__btn-primary" disabled={isSaving}><IoSave />{isSaving ? 'Enregistrement...' : 'Qualifier'}</button></div>
    </form>
  );
}
