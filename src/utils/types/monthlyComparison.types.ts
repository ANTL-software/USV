...
export type ComparisonMode = 'business' | 'aligned' | 'full';
...
export interface ComparisonPeriod {
  month: string; start: string; endExclusive: string; cutoffExclusive: string; cutoffTime: string | null;
  days: number; includedDates: string[]; basis: 'business' | 'calendar'; partialDay: boolean;
}
...
export interface ComparisonFilters { campaign: number; agent: number | null; month: string; reference: string; mode: ComparisonMode; }
export interface MonthlyComparison {
  campaign: ComparisonCampaign; agent: number | null; generatedAt: string;
  periods: { current: ComparisonPeriod; reference: ComparisonPeriod; mode: ComparisonMode; timezone: string; asOf: string };
